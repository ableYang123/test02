...
<%@ taglib uri="http://www.springframework.org/tags" prefix="spring"%>
	<form class="form-inline" id="editReportForm">
		<legend><spring:message code="report.edit.title"/></legend>
		<table>
			<tr>
				<td width="10%">
			...
			<tr>
				...
				<td>
					<div class="text-center">
						<button type="button" class="btn btn-default"><spring:message code="report.add.cancel"/></button>
	...
	<script type="text/javascript">
		...
		function editReport(param){
			$.assign({
				...
				"allowedReportTypes":param.reportType,
				"allowedReportSubTypes":param.reportSubTypes,
				"reportCategory":param.reportCategory,
				"description":param.description,
				"reportSql":param.reportSql,
			});
			$("#checkbox")[0].checked = param.useReport;
			var tdate = param.reportParameters;
			if(tdate != ''){
				$("#tableParameters").css('display','');
				for(var o in tdate){
					var value = tdate[o].reportParameterName;
					if(value==undefined)
						value="";
					$("#tableParameters-body")[0].innerHTML+='<tr name="'+tdate[o].id+'"><td name="'+tdate[o].parameterId+'">'+tdate[o].parameterName+'</td><td><input name="'+tdate[o].parameterName+'" value="'+value+'" class="form-control"/></td><td><button type="button" class="btn btn-link"><span class="glyphicon glyphicon-remove"></span></button></td></tr>';
				}
			}
		};
		...
		$('#tableParameters-body').on('click','button',function(param){
		...
		$.getWidget("saveBtn").click(function(){
			...
			if($("#tableParameters-body")[0].innerHTML != ''){
				...
				for(var i=0; i<len; i++) {
					...
					if($("#tableParameters-body tr").eq(i).attr('name') != undefined)
						json.id=$("#tableParameters-body tr").eq(i).attr('name');
					else
						json.id="";
