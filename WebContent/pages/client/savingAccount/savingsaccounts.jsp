...
// 		}
		...
		$("#modifyApplication").click(function(){
			if('${pd.pageType}' === 'center'){
				$.openWindow('<c:url value="/loadPage.do?nextPage=center.savingAccount.editCenterApplication"/>&savingsId=${pd.savingsId}&groupId=' + $('#clientName').attr('code') + '&withHoldTax=' + $('#clientName').attr('tax'),'iframe','90%','90%');
			}else if('${pd.pageType}' === 'group'){
				$.openWindow('<c:url value="/loadPage.do?nextPage=center.savingAccount.editCenterApplication"/>&savingsId=${pd.savingsId}&groupId=' + $('#clientName').attr('code') + '&withHoldTax=' + $('#clientName').attr('tax'),'iframe','90%','90%');
			}else{
				$.openWindow('<c:url value="/loadPage.do?nextPage=client.savingAccount.editSavingsAccounts"/>');
			}
			
		})
		
		$("#approve").click(function(){
			$.openWindow('<c:url value="/loadPage.do?nextPage=client.savingAccount.approveSaving"/>&pageType=${pd.pageType}');
		})
		
		$("#undoapproval").click(function(){
			$.openWindow('<c:url value="/loadPage.do?nextPage=client.savingAccount.undoapproval"/>&pageType=${pd.pageType}');
		})
		
		$("#assignStaffPend").click(function(){
			$.openWindow('<c:url value="/loadPage.do?nextPage=client.savingAccount.assignsavingsofficer"/>&pageType=${pd.pageType}');
		})
		...
		$("#rejectAccount").click(function(){
			$.openWindow('<c:url value="/loadPage.do?nextPage=client.savingAccount.rejectAccount"/>&pageType=${pd.pageType}');
		})
		
		
		$("#unassignStaffPend").click(function(){
			$.openWindow('<c:url value="/loadPage.do?nextPage=client.savingAccount.unassignsavingsofficer"/>&pageType=${pd.pageType}');
		})
		...
		$("#withdrawnByApplicant").click(function(){
			$.openWindow('<c:url value="/loadPage.do?nextPage=client.savingAccount.withdrawnByApplicant"/>&pageType=${pd.pageType}');
		})
		...
		$("#activeAccount").click(function(){
			$.openWindow('<c:url value="/loadPage.do?nextPage=client.savingAccount.activeAccount"/>&pageType=${pd.pageType}');
		});
		
		$("#postInterestAsOn").click(function(){
			$.openWindow('<c:url value="/loadPage.do?nextPage=client.savingAccount.postInterestAsOn"/>&pageType=${pd.pageType}');
		});
		
		$("#deposit").click(function(){
			$.openWindow('<c:url value="/loadPage.do?nextPage=client.savingAccount.accountDeposit"/>&pageType=${pd.pageType}');
		});
		
		$("#withdrawal").click(function(){
			$.openWindow('<c:url value="/loadPage.do?nextPage=client.savingAccount.accountWithdrawal"/>&pageType=${pd.pageType}');
		});
			...
			$.nest({
				...
				callback :function(param){
					...
					if(param != null || param.length > 0 || typeof param != 'undefined'){
						...
						window.location.href="loadPage.do?nextPage=client.savingAccount.savingsaccounts&pageType=${pd.pageType}&savingsId="+savingsId;
					}
				}
			}); 
		...
		$("#close").click(function(){
			$.openWindow('<c:url value="/loadPage.do?nextPage=client.savingAccount.closeSavingAccount"/>&pageType=${pd.pageType}');
		});
		
		
		$("#transferFunds").click(function(){
			$.openWindow('<c:url value="/loadPage.do?nextPage=client.savingAccount.accountTransferFunds"/>');
		});
		
		
		$("#addChargesPed").click(function(){
			$.openWindow('<c:url value="/loadPage.do?nextPage=client.savingAccount.accountAddCharges"/>&pageType=${pd.pageType}');
		})
		...
		function chargeDetil(ccccid){
			...
			$.openWindow('loadPage.do?nextPage=client.savingAccount.accountViewCharge');
		...
		function payCharge(chargesId){
			...
			$.openWindow('loadPage.do?nextPage=client.savingAccount.accountPayCharge')
		}
		
		function waiveCharge(chargesId){
			chargesViewId=chargesId;
			$.openWindow('loadPage.do?nextPage=client.savingAccount.accountWaiveCharge')
		}
		
		
		function editCharge(chargesId){
			chargesViewId=chargesId;
			$.openWindow('loadPage.do?nextPage=client.savingAccount.accountEditCharge')
		}
		
		
		function deleteCharge(chargesId){
			chargesViewId=chargesId;
			$.openWindow('loadPage.do?nextPage=client.savingAccount.accountDeleteCharge')
		}
		...
		function runreportSavings(trid){
			transactionId=trid;
			$.openWindow('loadPage.do?nextPage=client.savingAccount.accountRunreport');
		}
