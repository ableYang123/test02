	...
	<div id="informationPage" class="modal-body"
		...
		<div id="Metting" style="display: none">
			...
			<div class="modal-body" style="height: 53px; padding: 5px">
				<div class="col-md-6" style="padding: 5px">
					<div class="col-md-4" style="padding: 5px">
						<input type="checkbox" id="syncRepayments" checked="checked">
					</div>
					<div class="" style="padding-left: 0px">
						<label
							style="padding-top: 3px; font-size: 16px; font-weight: normal">
				...
				<div class="col-md-6" style="padding: 5px">
					<div class="col-md-4" style="padding: 5px">
						<input type="checkbox" id="syncDisbursement" name="syncDisbursement">
					</div>
					<div class="" style="padding-left: 0px">
						<label
							style="padding-top: 3px; font-size: 16px; font-weight: normal">
		...
		<div class="modal-body" style="height: 50px; padding: 5px">
			...
			<div class="col-md-6" style="padding: 5px">
				...
				<div class="col-md-2" style="padding-left: 0px; padding-top: 5px">
					<input id="makePaymentInstructions" name="makePaymentInstructions" type="checkbox">
				</div>
			</div>
		</div>
		<hr style="margin-top: 5; margin-bottom: 5; width: 868px">
		...
		<div class="modal-body" style="height: 50px; padding: 5px">
			...
			<div class="col-md-6" style="padding: 5px">
				...
				<div class="col-md-4" style="padding-left: 0px; padding-top: 5px">
					<input type="checkbox" id="calculateInterests" name="calculateInterests">
				</div>
			</div>
		</div>
		<hr style="margin-top: 5; margin-bottom: 5; width: 868px">
	...
	var LoanListUrl;
		...
		function initInformationPage(param) {
			...
			$.setSelect('loanOfficer', param.loanOfficerOptions, 'id',
					'displayName');
			$('#loanOfficer option[value="' + param.loanOfficerId + '"]').attr('selected', true);
			/* 贷款目的 */
			$("#loanPurpose").empty();
			$.setSelect('loanPurpose', param.loanPurposeOptions, 'id', 'name');
			$('#loanPurpose option[value="' + param.loanPurposeId + '"]').attr('selected', true);
			...
			$.assign({
				...
				'submittedOn'    : $.formatDate(param.timeline.submittedOnDate),
				'externalId'     : param.externalId,
				'firstRepaymentOn' : $.formatDate(param.expectedFirstRepaymentOnDate),
				'interestChargedFrom' : $.formatDate(param.interestChargedFromDate),
				'syncDisbursement' : param.syncDisbursementWithMeeting
				   })
		       /*判断同步付款日期与会议复选框按钮是否是选中状态  */
			if(param.syncDisbursementWithMeeting){
				 $("#syncDisbursement")[0].checked=true;	
			}else{
			     $("#syncDisbursement")[0].checked=false;	
			}
			   /*判断制定付款方面的常规说明复选框按钮是否是选中状态  */
			if(param.createStandingInstructionAtDisbursement){
				 $("#makePaymentInstructions")[0].checked=true;
			}else{
				$("#makePaymentInstructions")[0].checked=false;	
			}
			   /*判断计算在部分时间内确切日期的利息复选框按钮是否是选中状态  */
			 if(param.allowPartialPeriodInterestCalcualtion){
				$("#calculateInterests")[0].checked=true;
			}else{
				$("#calculateInterests")[0].checked=false;
			} 
			/* 贷款基金*/
			$("#fund").empty();
			...
			$.setSelect('linkSaving', param.accountLinkingOptions, 'id',
					'productName');
			$('#linkSaving option[value="' + param.linkedAccount.id + '"]').attr('selected', true);
			/* ------条款---- */
			/* 条款1-1贷款本金 */
			...
			/* 条款2-2.1每笔金额 */
			$("#repaidEver").val(param.repaymentEvery);
			/* 条款2-2.2每笔金额 */
			...
			checkMonth();
			/*获取抵押品数据  */
			$("#collateralList tbody").html("");
			getCollateral(param.collateral);
			showTable("collateralTable");
			setSubmmitMargin("collateralTable");
		...
		function initCalendarOptions(param) {
			...
			if (typeof (calendarOptions) != "undefined") {
				...
				for (var i = 0; i < calendarOptions.length; i++) {
					...
					var startDate = $.formatDate(calendarOptions[i].startDate);
		...
		/* 抵押品初始化 */
		function getCollateral(collateral) {
			if (collateral == null) {
				return 0;
			}
			for (var i = 0; i < collateral.length; i++) {
				var selectedCollateralValue = collateral[i].type.id;
				var selectedCollateralText = collateral[i].type.name;
				var collateralValue = collateral[i].value;
				var collateralDescription = collateral[i].description;
				addCollateralView(selectedCollateralValue,
						selectedCollateralText, collateralValue,
						collateralDescription);
			}
			showTable("collateral");
			setSubmmitMargin("collateral");
		}
		...
		function IntergrationCharges() {
			var json = [];
			$("#chargesList tbody").find("tr").each(function() {
				var tdArr = $(this).children();
				var chargeId = tdArr.eq(0).find('input').val();//chargeId
		...
		function IntergrationCollateral() {
			var json = [];
			$("#collateralList tbody").find("tr").each(function() {
				var tdArr = $(this).children();
				var dd = {};
				var type = tdArr.eq(0).find('input').val();//类型名称
		...
		function getAllValue(json) {
			...
			addValue("syncDisbursementWithMeeting", $.judgeCheckbox("syncDisbursement"), json);
