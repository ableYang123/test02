...
<form class="form-inline" id="editApplicationForm" naem="editApplicationForm" method="post">
	<fieldset>
	<legend>
		<spring:message code="label.center.centersave"/>
	</legend>
	<div id="selectIn">
		<table>
			<tr>
				...
				<th class="width36">
					<div class="form-group">
						<select id="prolist" name="prolist" class="form-control" style="width:170">
	...
	<div id="selectOn" style="display:none;">
		<table>
			<tr>
				<th class="width14"><spring:message code="label.center.products"/><label style="font-size: 14px; color: #d9534f">*</label></th>
				<th class="width36">
				...
				<th class="width14"><spring:message code="label.center.submitdate"/><span style="font-size: 14px; color: #d9534f">*</span></th>
				<th class="width36">
					<div class="form-group">
						<input type="text" datetype="date" name="submitDate" id="submitDate" local-date="true" afterToday='true' beforeToday="">
					</div>
				</th>	
			</tr>
			<tr>
	...
	<div class="text-right" style="padding-right: 80px">
		<button type="button" id="save" name="save" class="btn btn-primary" style="display:none;"><spring:message code="btn.submit"/></button>
	</div>
	</fieldset>
</form>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
				...
				$.nest({
					...
					data : {
						"dataType" : "GET",
						"mydata" : "",
						"url" : "api.v1.savingsaccounts.template?groupId="+ groupId +"&staffInSelectedOfficeOnly=true"
					},
					callback : "initback"
			});
				
				$.getWidget('centerName').click(function(){
					window.location.href = "loadPage.do?nextPage=center.detail&centerId="+groupId;
				});
				/* prolist产品change事件 */
				$.getWidget('prolist').change(function(){
					var productId = parseInt($.value('prolist'));
				...
				$('#withHoldTax').click(function(){
					var thLast = $(this).closest('tr').children(':last');
					if(!$(this).is(':checked')){
						thLast.hide();
					}else{
						thLast.show();
		...
		function load(groupId,productId){
			$.nest({
				...
				data : {
					"dataType" : "GET",
					"mydata" : "",
					"url" : "api.v1.savingsaccounts.template?groupId="+ groupId +"&productId="+productId+"&staffInSelectedOfficeOnly=true"
				},
				callback : "initback"
			});
		}
		...
		function callback2(param) {
			//alert(param.savingsId);
			$.setReadOnly("editApplicationForm");
			$.bAlert($.fn.bootstrapValidator.i18n.common.tipSaveSuccessful);
			window.location.href="loadPage.do?nextPage=client.savingsaccounts&pageType=center&savingsId="+param.savingsId;
		}
		
 		function initback(param){
 			if(begin){
 				$.setSelect('prolist', param.productOptions, 'id', 'name');
 				...
 				if(param.withHoldTax){
 					$('#withHoldTax').closest('tr').show();
 					$('#withHoldTax').prop('checked',true);
 					$('#taxGroup').text(param.taxGroup.name);
 				}else{
 					$('#withHoldTax').closest('tr').hide();
 					$('#withHoldTax').prop('checked',false);
 					$('#taxGroup').text('');
 				}
 				
 				if(param.minBalanceForInterestCalculation){
 					$('#minBalanceForInterestCalculation').closest('tr').show();
 					$('#minBalanceForInterestCalculation').text(param.minBalanceForInterestCalculation);
 				}else{
 					$('#minBalanceForInterestCalculation').closest('tr').hide();
 					$('#minBalanceForInterestCalculation').text('');
 				}
 				//费用
 		        var chargeoptions = [];
 		        var currencyName = param.currency.name;
 		        ...
 		        if(typeof(param.charges) != 'undefined' && param.charges.length > 0){
 		        	...
 		        	$.each(param.charges,function(index,item){
 						...
 						if('Specified due date' === item.chargeTimeType.value){
 							str += '<td><input type="text" datetype="date" name="specifiedDate" local-date="" afterToday="" beforeToday="" /></td>';
 							$.datetimepicker();
 							$.assign({
 								"specifiedDate":$.formatDate(item.dueDate,'yyyy-MM-dd')
 							});
 						}else{
 							str += '<td />';
 						}
 				...
 				if(typeof(param.timeline.submittedOnDate) != 'undefined'){
 					$.assign({
 						 "submitDate" : $.formatDate(param.timeline.submittedOnDate,'yyyy-MM-dd')
 					});
 				}
