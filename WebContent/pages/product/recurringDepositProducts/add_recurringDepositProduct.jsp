...
<form class="form-horizontal" id="addRecurDPForm" name="addRecurDPForm">
	<fieldset>
		<legend>
			<spring:message code="deposit.title.recurring.create" />
		</legend>
		...
		<div class="form-group">
			<label for="isMandatoryDeposit" class="col-sm-2 control-label"><spring:message code="deposit.label.isMandatoryDeposit" /></label>
			<div class="col-sm-3">
				<label class="checkbox"> <input id="isMandatoryDeposit" type="checkbox"></label>
			</div>
			<label for="adjustAdvanceTowardsFuturePayments" class="col-sm-3 control-label"><spring:message code="deposit.label.adjustAdvanceTowardsFuturePayments" /></label>
			<div class="col-sm-3">
				<label class="checkbox"> <input id="adjustAdvanceTowardsFuturePayments" type="checkbox"></label>
			</div>
		</div>
		<div class="form-group">
			<label for="allowWithdrawal" class="col-sm-2 control-label"><spring:message code="deposit.label.allowWithdrawal" /></label>
			<div class="col-sm-3">
		...
		<div class="form-group">
			<label for="minBalanceForInterestCalculation" class="col-sm-2 control-label"><spring:message code="deposit.label.minBalanceForInterestCalculation" /><span style="color: #d9534f;">*</span> </label>
			<div class="col-sm-3">
				<input type="text" class="form-control" id="minBalanceForInterestCalculation" name="minBalanceForInterestCalculation" />
			</div>
		</div>
		...
		<div class="form-group">
			<label for="fromDate" class="col-sm-2 control-label">
				<spring:message code="deposit.label.fromDate" /><span style="color: #d9534f;">*</span>
			</label>
			<div class="col-sm-3">
				<input type="text" datetype="date" name="fromDate"  local-date="true"   beforeToday="false"/>
			</div>
		...
		<div class="form-group">
			...
			<spring:message code="deposit.label.isPrimaryGroupingByAmount" /><span style="color: #d9534f;">*</span>
			</label>
			<div class="col-sm-1">
	...
	$(document).ready(
			function() {
				...
				$.valid("addRecurDPForm", 
				 ...
				 $.nest({
					...
					data : {
						"dataType" : "GET",
						"mydata" : '',
						"url" : "api.v1.recurringdepositproducts.template"
					},
					callback : "add_back"
				});
				...
				$.getWidget("save").click(
										function() {
											if ($.validateForm("addRecurDPForm")) {
												//detail
												var name = $('#name').val();
												...
												/*与定期存款数据不同之处*/
												var isMandatoryDeposit=$('#isMandatoryDeposit').is(':checked');
												var adjustAdvanceTowardsFuturePayments	=$('#adjustAdvanceTowardsFuturePayments').is(':checked');
												var allowWithdrawal=$('#allowWithdrawal').is(':checked');
												json["isMandatoryDeposit"] = isMandatoryDeposit;
												json["adjustAdvanceTowardsFuturePayments"] = adjustAdvanceTowardsFuturePayments;
												json["allowWithdrawal"] = allowWithdrawal;
												
												var minBalanceForInterestCalculation=$('#minBalanceForInterestCalculation').val();
												json["minBalanceForInterestCalculation"] = minBalanceForInterestCalculation;
												...
												$('#chartTable').find('tr').eq(1).nextAll().each(function() {
													var amountRangeFrom = $(this).find('input[name="amountRangeFrom"]').val();
													var amountRangeTo = $(this).find('input[name="amountRangeTo"]').val();
													var fromPeriod = $(this).find('input[name="fromPeriod"]').val();
													var toPeriod = $(this).find('input[name="toPeriod"]').val();
													
													var annualInterestRate = $(this).find('input[name="annualInterestRate"]').val();
										...
										$.nest({
													...
													data : {
														...
														"dataType" : "POST",
														"mydata" : json,
														"url" : "api.v1.recurringdepositproducts"
													},
