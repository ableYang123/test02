...
<form class="form-horizontal" id="editFloatingRatesForm"
	name="editFloatingRatesForm">
	<fieldset>
		<legend>
...
<%-- 			<button type="button" class="btn btn-primary" name="addFloatingRate" onclick="addPeriod();" id="addFloatingRate"><spring:message code="lable.add"/></button>
 --%>		
...
<script type="text/javascript">
	$(document).ready(function() {
			...
			$.getWidget("save").click(
										function() {
											if ($.validateForm("editFloatingRatesForm")) {
	...
	function editback(param) {
		...
		if (periodsArray.length > 0) {
			for (var i = 0; i < periodsArray.length; i++) {
				...
				$('#periodTable').find('tr').eq(i + 2).find('input[name="fromDate"]').val(fromDate);
				$('#periodTable').find('tr').eq(i + 2).find('input[name="interestRate"]').val(periodsArray[i].interestRate);
	...
	function edit_success(param) {
		$.setReadOnly('editFloatingRatesForm');
