...
<form class="form-horizontal"  id=FrequentPostings name="FrequentPostings">
	<div class="container">
<fieldset>
		<legend>
			<spring:message code="Frequent.Postings" />
		</legend>
		...
		<div class="form-group">
			...
			<label for="rule" class="col-sm-2 control-label"><spring:message code="Accounting.rules" /><span style="color: #d9534f;">*</span> </label>
			<div class="col-sm-3">
				<select id="rule" name="rule" class="form-control" style="width: 198px;">
				<option value="rule"><spring:message code="Select.Code"/></option>
				</select>
			</div>
			</div>
			<div class="form-group">
	...
	<td>
		...
		function getRules(param) {
			$.setSelect('rule', param, 'id', 'name');
			$.setSelect('debits', param[0].debitAccounts, 'id', 'name');
			$.setSelect('debits2', param[0].creditAccounts, 'id', 'name');
		}
			...
			$("#del").hide();
			 ...
			 $.nest({
					...
					data : {
						"dataType" : "GET",
						"mydata" : "",
						"url" : "api.v1.accountingrules"+"?associations=all"
					},
					callback : "getRules"
				}); 
...
$.getWidget("rule").change(function() {	
	if(this.value)
		checkSelect(this.value);

});
function checkSelect(value){
	var selectedOption = document.getElementById("rule").value;
