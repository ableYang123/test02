...
(function ($) {
	...
	$.addFlex = function (t, p) {
		...
		p = $.extend({ //apply default properties
			height: 'auto', //default height 插件的高度，单位为px
			width: 'auto', //auto width 自适应宽度
			striped: true, //apply odd even stripes 是否奇偶交互
			novstripe: false,
			minwidth: 30, //min width of columns
			minheight: 80, //min height of columns
			resizable: true, //allow table resizing 表格是否可伸缩
			url: false, //URL if using data from AJAX
			method: 'POST', //data sending method
			dataType: 'json', //type of data for AJAX, either xml or json
			singleSelect : true, // 默认为单选
			errormsg: 'Connection Error',
			usepager: true, //是否分页 false->true
			nowrap: true, //是否不换行
			page: 1, //current page 默认当前页
			total: 1, //total pages 总页面数
			pcpager: false, //PC端是否单独做分页 false->true
			limit:15,//客户端分页默认记录数
			dataList:[],//客户端总记录数据
			search : false,//PC端是否单独做搜索 false->搜索列的名称
			searchList:[],//用于客户端搜索的原始数据
			...
			query: '',	//搜索查询条件
			qtype: '', //搜索查询的类别
			nomsg: 'No items', //没有符合条件的记录存在
			minColToggle: 1, //minimum allowed column to be hidden 允许显示的最小列
			showToggleBtn: false, //show or hide column toggle popup 是否允许显示隐藏列
		...
		var g = {
						...
						$('thead tr:first th', g.hDiv).each( //add cell
							function () {
								...
								if (typeof row.cell == 'undefined') {
									//row[p.colModel[idx].name];
									if($(this).attr('type') == 'datetime'){
										var tdvalue = row[$(this).attr('name')];
										if($.isEmpty(tdvalue)){
											tdvalue = eval("row."+$(this).attr('name'));
										}
										if(tdvalue == undefined){
											tdvalue = "";
										}										
										td.innerHTML = $.formatDate(tdvalue,'','hh:mm:ss');
										...
										if($.isEmpty(tdvalue)){
											...
											td.innerHTML = $.formatDate(tdvalue);
										}
										
									}else if($(this).attr('type') == 'action'){
										var tdvalue = eval("row."+$(this).attr('name'));
										console.log(tdvalue);
										if($.isEmpty(tdvalue)){
											//tdvalue = eval("row."+$(this).attr('name'));
											td.innerHtml = '';
											...
											var button;
											if($(this).attr('button') == 'true'){
												$(td).append('<input type="button" value="'+tdvalue+'">');
												button=$(td).find("input:first");
											}
											var actionjs=$(this).attr('actionjs');
											eval(actionjs);
										}
									}else {
										if(new RegExp('.').test($(this).attr('name'))){
											var tdhtml = eval("row."+$(this).attr('name'));//row[$(this).attr('name')];
										}else{
											var tdhtml = row[$(this).attr('name')];//eval("row."+$(this).attr('name'));
										}
										if($.isEmpty(tdhtml)){
											tdhtml = eval("row."+$(this).attr('name'));
										}
										if(tdhtml == undefined){
											tdhtml = "";
										}
									...
									/*td.value = row[$(this).attr('name')];//eval("row."+$(this).attr('name'));
									if($.isEmpty(td.value)){
										td.value = eval("row."+$(this).attr('name'));
									}*/
			...
			newDataList : function(param){
				var newList = new Array();
				var start = p.limit*(p.page-1);
				var end = p.limit*p.page;
			...
			addRows : function(param,search) {
				...
				if(p.pcpager){
					...
					if(p.dataList.length>0){
						p.total =parseInt(p.dataList.length/p.limit);
						if((p.dataList.length/p.limit)>p.total){
							p.total++;
						}
						LData = this.newDataList(p.dataList);
					}
				}
				g.addData({rows: LData, page: p.page, total: p.total});
				this.setPage(p.page, p.total);
			},
			setPage : function(page,pages){	
				if(page != undefined && pages != undefined){
					if("0000" == pages || "0" == pages){					
						p.page = 0;
					}else{
						p.page = page;
					}				
					p.pages = pages;
					if(p.usepager){
						$('.pcontrol input', this.pDiv).val(parseInt(p.page));
						$('.pcontrol span', this.pDiv).html(parseInt(p.pages));
						if(p.pages <=1){
			...
			changePage: function (ctype) { //change page
				...
				switch (ctype) {
					...
					case 'prev':
						...
						break;
					case 'next':
						if (parseInt(p.page) < parseInt(p.pages)) {
							p.newp = parseInt(p.page, 10) + 1;
						}
						break;
					case 'last':
				...
				if (p.newp == p.page || $.isEmpty(p.newp)) {
					return false;
				}
				if(p.pcpager && p.dataList.length>0 ){
					p.page = p.newp;
					this.addRows(p.dataList,"search");
				}else if(p.nestjson){
					if(p.nestjson.data != undefined){
						$.extend(true,p.nestjson.data,{newpage : p.newp});
						$.nest(p.nestjson);
					}
				}
				/*
			...
			addRowProp: function () {
				...
				$('tbody tr', g.bDiv).each(
						function() {
							...
							/*添加多选框 修改2 start */
							//为数据区每行添加checkbox，并且选中checkbox同时可选中行
							if (p.singleSelect == false) {
									var cth = $('<td/>');			 //必须是th，否则checkbox列会参与交换列，引起样式错位						
									var cthch = $('<input type="checkbox"/>');									
									var objTr = $(this);   //指行对象
									$(cthch).click(function(){
									  	 if(this.checked){  //this指checkbox
												   objTr.addClass('trSelected');
												}	else{
												   objTr.removeClass('trSelected');
												   selectCount = selectCount - 1;
												   if($('thead tr:first td', g.hDiv).find('input')[0].checked == true) {
	   													$('thead tr:first td', g.hDiv).find('input')[0].checked = false;
													 } 
										   }							
									});
									cth.attr({ width: "22"}).append(cthch);
									$(this).prepend(cth);					
							}
	            /*添加多选框 修改2 end */
				});
				...
				$('tbody tr', g.bDiv).on('click', function (e) {
					...
					if(p.singleSelect == false){
					  	if($(this).hasClass('trSelected')){  	//选中行的时候同时选中checkbox
			...
			pager: 0,
			getSaerchTh : function(){
				var searchJson = false;
				$('thead tr:first th:visible').each(function() {
					if(!$.isEmpty($(this).attr("search"))){
						if(!searchJson){
							searchJson = {};
						}
						searchJson[$(this).attr("name")] = $(this).html().trim();
					}
				});
				p.search = searchJson;
			}
		...
		//pc端搜索
		if(p.search){
			var tDiv20 = document.createElement('div');
			tDiv20.className = 'tDiv20';	
			var placeholderValue = "";
			$.each(p.search, function(x, n) {
			...
			$(tDiv20).append("<input type='text' id='searchTh' name='searchTh' placeholder='按"+placeholderValue+"筛选' style='width:100%;height:30px;' class='form-control' />");
			$(g.tDiv).append(tDiv20);
			$('input[name=searchTh]', g.tDiv).keyup(function (e) {
				var searchdata = p.searchList;
				var searchValue = $.trim($.value("searchTh"));
				var newDataList = new Array();
				if(!$.isEmpty(searchValue)){
					for(var i=0;i<searchdata.length;i++){
						var rowJson = searchdata[i];
						var isPush = false;
						$.each(p.search, function(x, n) {
							var tdvalue = rowJson[x]
							if(!$.isEmpty(tdvalue)){
								if(tdvalue.toLowerCase().indexOf(searchValue.toLowerCase())>-1){
									isPush = true;
								}
							}
						});
						if(isPush){
							newDataList.push(rowJson);
						}
					}
				}else{
					newDataList = searchdata;
				}
				g.addRows(newDataList, "search");
			});
		...
		if (p.buttons) {
			...
			for (var i = 0; i < p.buttons.length; i++) {
				...
				if (!btn.separator) {
					...
					btnDiv.selected = btn.selected;
					btnDiv.bclass = btn.bclass;
					if (btn.id) {
						btnDiv.id = btn.id;
					}
					if (btn.onpress) {
						$(btnDiv).click(function () {
							if(this.bclass != 'add' && this.selected  == undefined ){
								if($('.trSelected',g.gDiv).length<=0){
									$.bAlert($.fn.bootstrapValidator.i18n.flexigrid.datagridSelectData);
									return false;
								}
							}	
		...
		/*begin-- by bihuanyu*/
		var tableWidth=$(g.hDiv).width();
		var tableLength=$('thead tr:first th', g.hDiv).not("[style]").length;
		/*end-- by bihuanyu*/
		...
		$('thead tr:first th', g.hDiv).each(function (i) {
			...
			if($(this).css("display")!='none'){
				...
				if(tableLength==1 && tableWidth>0){
					//this.width=parseInt(this.width)+tableWidth;
		...
		if ($('th', g.hDiv).length) {
	       /*添加多选框 修改4 start*/
	       //增加表头checkbox，提供全选、全不选功能
					if (p.singleSelect == false) {
								var cth = $('<td/>');	 //必须是td，否则数据区数据挤一块，无样式
								var cthch = $('<input type="checkbox"/>');
								$(cthch).click(function(){
									  		if(this.checked){
									  			  if($('tbody tr',g.bDiv).length > p.maxSelectCount){
										  			  	this.checked = false;
														  	bAlert(jQuery.validator.messages.maxSelectCount);
														  	return;
													  }
								...
								cth.attr({ width: "22" }).append(cthch);
							  $('thead tr:first', g.hDiv).prepend(cth);
					};
	       /*添加多选框 修改4 end*/
	...
	$.fn.addRows = function(param) { // 用于嵌套到列表
		return this.each(function() {
			
			if (this.grid)
				this.grid.addRows(param);
		});
	};

	$.fn.setPage = function(pg,pgs){
		   this.each(function() {
				if (this.grid)
					this.grid.setPage(pg,pgs);
			});
		};
		
	$.fn.colVal = function(param) {	  //gao_qk 用于提出选中行的某一列
		var row = $(this).find("tr[class*='trSelected']");
		var index = $("table").index($(this));
		var tables = $("table");
		var headTb = $(tables[index - 1]).find('thead');
		var cols = $(headTb[0]).find('th');
		for ( var i = 0; i < cols.length; i++) {
			var colName = $(cols[i]).attr('name');	
			if(param == colName){
				return row.find("td div").eq(i).html();
			}
		}		
	};	

	$.fn.getColValues = function(rows,name) {
	   	var index = $("table").index($(this));
	   	var tables = $("table");
	   	var headTb = $(tables[index - 1]).find('thead');
	   	var cols = $(headTb[0]).find('th');
	   	...
	   	if(rows) {
	   	   for(var i=0; i<rows.length; i++) {
	   	      ...
	   	      for(var j=0; j<cols.length; j++) {
	   	      	var colName = $(cols[j]).attr('name');
	   	      	if(name == colName) {
	   	      	    colValue = $(rows[i]).find("td div").eq(j).html();	
	   	      	}
	   	      }	
	...
	$.fn.getSelectRows = function() {
	    var rows = $(this).find("tr[class*='trSelected']");
	    var newRows = new Array();
	    if(rows.length > 0){
	    	for(var i=0;i<rows.length;i++){
	    		if($(rows[i]).attr("style")==""){
	    			newRows.push(rows[i]); 
	    		}
	    	}
	    }
